...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--mobile', required=False, help='Mobile number (10 digits)')
        parser.add_argument('--force', action='store_true', help='If set, reset password for existing user')
    ...
    def handle(self, *args, **options):
        ...
        if existing:
            if options.get('force'):
                ...
                existing.save()
                self.stdout.write(self.style.SUCCESS(f'Updated password for existing user: {existing.pk} {existing.email}'))
                return
            else:
